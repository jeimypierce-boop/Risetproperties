...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $property_data['availability_date'] = trim($_POST['availability_date'] ?? '');
    ...
    if ($property_data['unit_count'] <= 0) {
        $errors[] = 'Total units must be at least 1';
    }
    if (empty($property_data['availability_date'])) {
        $errors[] = 'Availability date is required';
    } elseif (!DateTime::createFromFormat('Y-m-d', $property_data['availability_date']) || DateTime::createFromFormat('Y-m-d', $property_data['availability_date'])->format('Y-m-d') !== $property_data['availability_date']) {
        $errors[] = 'Availability date must be a valid date in YYYY-MM-DD format';
    }

    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO properties (title, description, property_type, category, bedrooms, bathrooms, square_feet, unit_count, price, currency, location, city, country, status, contact_person, contact_phone, contact_email, availability_date, landlord_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        $landlord_id = isset($_SESSION['user_id']) ? intval($_SESSION['user_id']) : NULL;
        $stmt->bind_param('ssssiiiidsssssssssi', 
            $property_data['title'], 
        ...
        if ($stmt->execute()) {
            ...
            $errors[] = 'Error creating property: ' . $stmt->error;
        }
        $stmt->close();
    }
}
...
<html lang="en">
<head>
    <title>New Property - Riset Property Management</title>
    <meta charset="utf-8">
            ...
            <div class="sb2-1">
                <div class="sb2-12">
                    <ul>
                        ...
                        <li><h5>Add Property <span>New Listing</span></h5></li>
                    </ul>
                </div>
                <div class="sb2-13">
            ...
            <div class="sb2-2">
                <div class="sb2-2-2">
                    <ul>
                        ...
                        <li class="active-bre"><a href="#">Add Property</a></li>
                    </ul>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="box-inn-sp admin-form">
                                <div class="inn-title">
                                    <h4>New Property</h4>
                                    <p>Add a new property to your portfolio with all relevant details.</p>
                                </div>
                                <div class="tab-inn">
                                    ...
                                    <form method="post">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="input-field col s6">
                                                <input id="availability_date" name="availability_date" type="date" value="<?php echo htmlspecialchars($property_data['availability_date']); ?>" class="validate" required>
                                        ...
                                        <div class="row">
                                            <div class="input-field col s12">
                                                <button type="submit" class="waves-effect waves-light btn-large">Create Property</button>
